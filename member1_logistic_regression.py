...
Algorithm: Multinomial Logistic Regression (Baseline Benchmark Node)
...
def run_member1_pipeline():
    ...
    print(" [MEMBER 1] RUNNING MULTINOMIAL LOGISTIC REGRESSION PIPELINE")
    ...
    # FIXED: Removed 'multi_class' to comply with newer scikit-learn versions.

    # 'lbfgs' automatically handles multinomial multiclass optimization.
    ...
    print("      MEMBER 1 EXPERIMENTAL LOGS")

    print("#"*40)

    print(f"• Model Convergence Latency  : {training_latency:.6f} Seconds")
    ...
    print("[SAVED] Exported 'logistic_regression_model.pkl' successfully.")
    ...
    plt.plot(train_sizes, np.mean(train_scores, axis=1), 'o-', color='b', label='Training Profile Metric')

    plt.plot(train_sizes, np.mean(test_scores, axis=1), 'o-', color='orange', label='Stratified CV Metric')

    plt.title('Member 1: Logistic Regression Empirical Learning Curves')
    ...
    plt.savefig('member1_learning_curve.png', dpi=300)

    print("[SAVED] Learning curve plot exported as 'member1_learning_curve.png'.")
