...
Algorithm: Random Forest Classifier (Tree-Based Ensemble Node)
...
def run_member3_pipeline():
    print("="*70)
    print(" [MEMBER 3] RUNNING RANDOM FOREST ENSEMBLE PIPELINE")
    print("="*70)
    ...
    # n_estimators sets 100 decision trees to ensure ensemble stability,
    # max_depth limits over-parameterization, and n_jobs=-1 enforces maximum parallel core scaling.
    ...
    print("\n" + "#"*40)
    print("      MEMBER 3 EXPERIMENTAL LOGS")
    print("#"*40)
    ...
    print("[SAVED] Exported 'random_forest_model.pkl' successfully.")
    ...
    plt.plot(train_sizes, np.mean(train_scores, axis=1), 'o-', color='g', label='Training Profile Metric')
    plt.plot(train_sizes, np.mean(test_scores, axis=1), 'o-', color='red', label='Stratified CV Metric')
    plt.title('Member 3: Random Forest Empirical Learning Curves')
    plt.xlabel('Volumetric Sample Training Dimensions')
    plt.ylabel('Mathematical Accuracy Score Metric')
    plt.legend(loc='best')
    plt.grid(True)
    plt.savefig('member3_learning_curve.png', dpi=300)
    print("[SAVED] Learning curve plot exported as 'member3_learning_curve.png'.")
    print("="*70)
